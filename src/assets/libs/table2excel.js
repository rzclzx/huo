        ...
        while (i < len) {
            if (i === -1) {
                Array.from(tableHead.rows[0].children).forEach((td) => {
                    let _html = td.children[0].children[0]? td.children[0].children[0].innerHTML : td.children[0].innerHTML
                    tableInnerHTML = tableInnerHTML + '<th>' + _html + '</th>';
                });
                ...
                Array.from(tableBody.rows[i].children).forEach((td) => {
                    let _html = td.children[0].children[0]? td.children[0].children[0].innerHTML : td.children[0].innerHTML
                    tableInnerHTML = tableInnerHTML + '<td>' + _html + '</td>';
                });
                tableInnerHTML += '</tr>';
            }
            i++;
