        ...
        on: {
            'click': () => {
                if (!currentRow.editting) {
                    ...
                    let old = JSON.parse(JSON.stringify(vm.thisTableData));
                    vm.thisTableData = JSON.parse(JSON.stringify(vm.edittingStore));
                    ...
                    vm.$emit('on-change', vm.handleBackdata(vm.thisTableData), index, old);
                }
            }
        }
    ...
    return h('Poptip', {
        ...
        on: {
            'on-ok': () => {
                ...
                vm.$emit('input', vm.handleBackdata(vm.thisTableData));
                vm.$emit('on-delete', vm.handleBackdata(vm.thisTableData), index, old);
            }
        }
    }, [
        h('Button', {
    ...
    return h('Button', {
        ...
        on: {
            click: (event) => {
                ...
                vm.thisTableData = JSON.parse(JSON.stringify(vm.edittingStore));
                vm.$emit('input', vm.handleBackdata(vm.thisTableData));
                vm.$emit('on-cell-change', vm.handleBackdata(vm.thisTableData), param.index, param.column.key, old);
            }
        }
    });
};
