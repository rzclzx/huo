...
<template>
    <div>
        <Row>
            ...
            <Button type="primary" @click="search" v-if="$store.state.user.checkPerm('job:hbhadaxsymbols:list')">{{$t('查询')}}</Button>
        </Row>
...
export default {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: (h, params) => {
                      ...
                      if ( this.$store.state.user.checkPerm('job:hbhadaxsymbols:update') ) {
    ...
    methods: {
        ...
        async getCurrencys(){
          const res= await this.$axios(legalTender.getHadaxCurrencyList)
          ...
          const title = item.krwVisible===1?this.$t('开启交易对'):this.$t('关闭交易对')
                this.$Modal.confirm({
                    ...
                    async onOk(){
                            ...
                            const obj = {...legalTender.updateHadax,data:{id:item.id,krwVisible:status}}
