        ...
        methods: {
            ...
            save() {
                if (this.$store.state.user.checkPerm('withdraw:refuse:edit')) {
                    ...
                    query.refuseEdit.data = {...params};
                    this.$axios(query.refuseEdit).then((res) => {
            ...
            return {
                ...
                delConfig: {
                    ...
                    onOk: () => {
                        if (this.$store.state.user.checkPerm('withdraw:refuse:del')) {
                            query.refuseDel.params.id = this.formData.id;
                            this.$axios(query.refuseDel).then((res) => {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: (h, {row}) => {
                            const doms = [];
                            if (this.$store.state.user.checkPerm('legal:withdrawls:refuse:edit')) {
                                doms.push (h('a', {
                                    props: {
                                        type: 'primary',
                                        size: 'small'
                            ...
                            if (this.$store.state.user.checkPerm('legal:withdrawls:refuse:del')) {
                                doms.push(h('a', {
                                    props: {
                                        type: 'primary',
                                        size: 'small'
