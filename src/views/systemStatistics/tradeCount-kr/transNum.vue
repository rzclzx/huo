<template lang="html">
    <Row>
        ...
        <Row style="margin-top: 20px" v-if="$store.state.user.checkPerm('statistics:getTradeNumber')">
            <DatePicker v-model="searchDate" format="yyyy/MM/dd"  :transfer="true" type="date" :options="chooseDate" :placeholder="$t('一周数据 - 起  -  一周数据 - 止')" style="width: 200px"></DatePicker>
        ...
        <Card style="margin-top: 20px">
            ...
            <Button slot="extra" type="primary" v-if="$store.state.user.checkPerm('statistics:getTradeNumberExport')" @click.prevent="dataExport">{{$t('数据导出')}}</Button>
...
export default {
    ...
    mounted () {
        if ( this.$store.state.user.checkPerm('statistics:getTradeNumber') ) {
            this.getColumns()
    ...
    computed: {
        radioText () {
            return [this.$t('USDT 盘'), this.$t('BTC 盘'), this.$t('ETH 盘')]
        }
    },
    methods: {
        // 获取近7天表头
