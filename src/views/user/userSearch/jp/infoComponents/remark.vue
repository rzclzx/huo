...
<template>
    <div>
        <div class="flex-start-center">
            ...
            <Button type="primary" @click="add" v-if="$store.state.user.checkPerm('user:remark:type:add')">{{$t('增加')}}</Button>
        </div>
        ...
        },
        {
            ...
            render: (h, params) => {
                return h('Button', {
                    props: {
                        type: 'primary',
                        disabled: !vm.$store.state.user.checkPerm('user:mgt:query:note:list')
                    },
                    on: {
                        click: () => {
                            vm.$Modal.confirm({
